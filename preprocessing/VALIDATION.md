...
## Trimble T02 -> RINEX wrapper validation

The publication-oriented `convert_t02_to_rinex.py` wrapper was spot-checked
using the KOH2 hourly raw file:

```text
25010100A.T02
```

The Python wrapper called the locally installed Trimble ConvertToRINEX program
using the same invocation pattern as the historical batch workflow:

```text
convertToRINEX.exe 25010100A.T02 -p <output-directory>
```

The wrapper-generated observation file:

```text
25010100A.25o
```

was compared with a second file generated by executing the same Trimble
ConvertToRINEX command manually on the same T02 input.

SHA-256 was computed for the RINEX observation body after `END OF HEADER`.
Both products produced:

```text
82f20d20170c08021ae8e42df1d56256f7d8799f00296cdbad81925317e5345e
```

Result: **bit-for-bit identical observation body** between the Python wrapper
and the direct Trimble ConvertToRINEX command.

## Complete preprocessing validation status

The publication-oriented preprocessing code has now been spot-checked
component by component on representative KOH2 files:

- Trimble T02 -> RINEX wrapper:
  validated on `25010100A.T02` (2025-01-01, 00 UTC);
- legacy RINEX 2 -> RINEX 3 + KOH2 CRUX:
  validated on `koh23620.19o`;
- hourly 1 Hz -> daily 1 s -> daily 30 s:
  validated on 2019-12-28 (DOY 362);
- hourly 10 Hz -> daily 1 s -> daily 30 s:
  validated on 2025-01-01 (DOY 001).

For each tested wrapper stage, the generated RINEX observation body was
bit-for-bit identical to the appropriate reference output generated by the
corresponding operational/direct command.

These are representative spot checks and do not imply that every observation
day in the full 2019-2026 archive was independently checksum-validated.
